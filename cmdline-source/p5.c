 ...
 * To run: protocol5 events timeout  pct_loss  pct_cksum  debug_flags multiple
 *
 * Written by Andrew S. Tanenbaum
 * Revised by Shivakant Mishra
 * Revised by Stephen W. Turner
 */
...
int main(int argc, char *argv[])
{
  ...
  if (!parse_first_six_parameters(argc, argv, &event, &timeout_interval,
    ...
    printf ("Usage: protocol5 events timeout loss cksum debug Datafile-flag\n");
    exit(1);
  }

  printf("\n\n Simulating Protocol 5\n");
  start_simulator(protocol5, protocol5, event, timeout_interval, pkt_loss, garbled, debug_flags, multiples);
}
