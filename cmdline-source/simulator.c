 ...
 *    Second revision by Stephen W. Turner, June 2010.
 */
...
/* Prototypes. */
void start_simulator(void (*p1)(), void (*p2)(), long event, int tm_out, int pk_loss, int grb, int d_flags, int the_file);
...
int parse_first_six_parameters(int argc, char *argv[], long *event, int *timeout_interval, int *pkt_loss, int *garbled, int *debug_flags, int *the_file);
 ...
 */
  ...
  append_file = the_file;		/* Determine whether part of multi-run or just single */
  ...
  printf("\n\nEvents: %u    Parameters: %u %d %u\n",
      (unsigned int) last_tick/DELTA, (unsigned int) timeout_interval/DELTA, (unsigned int) pkt_loss/10, (unsigned int) garbled/10);
  ...
  while (tick <last_tick) {
	...
	if (word == NOTHING) { hanging[process] += DELTA; ncount++; }
...
/*		close(w1);
		...
		pwfd = w2;	/* fd for writing frames to worker 0 */
sleep(2);
...
/*	close(r1);
	...
	pwfd = w1;	/* fd for writing frames to worker 1 */
sleep(2);
...
/* End the simulation run by sending each worker a 32-bit zero command. */
  ...
  /* Remove whatever is there from before */
  ...
  /* This SHOULD only have the actual statistics "word" array from the	*/
  /* print_statistics routine, at this point, due to the two reads	*/
  /* about 10 lines above here.						*/
  /**********************************************************************/
  n = read(r4, res1, MANY*sizeof(int));
  k1 = 0;
  while (res1[k1] != 0) k1++;
 ...
 bigint ct, word = OK;
  ...
  while (true) {
	...
	if ((debug_flags & PERIODIC) && (tick%INTERVAL == 0))
		printf("Tick %u. Proc %d. Data sent=%d  Payloads accepted=%d  Timeouts=%d\n", (unsigned int) tick/DELTA, id, data_sent, payloads_accepted, timeouts);
	...
	if (*event == timeout) {
		...
		if (debug_flags & TIMEOUTS)
		      printf("Tick %u. Proc %d got timeout for frame %d\n",
					       (unsigned int) tick/DELTA, id, oldest_frame);
	}
 ...
 */
  ...
  rc = select(prfd+1,&fds,NULL,NULL,&tv);
  if (rc < 0) sim_error("Cannot select peer pipe");

  /* If select tells us that there's data to read, then go into */
  /* a while loop to read as many as there are.			*/

  if (FD_ISSET(prfd,&fds)) {

	/* How many frames can be read consecutively? */
	top = (outp <= inp ? &queue[MAX_QUEUE] : outp);/* how far can we rd?*/

        /* As long as there's data to read, try to read it. */
	while (rc) {

          if (nframes >= MAX_QUEUE)      /* check for possible queue overflow*/
                sim_error("Out of queue space. Increase MAX_QUEUE and re-make.");

	  /* Read just one frame at a time */
	  if (read(prfd, inp, FRAME_SIZE) != FRAME_SIZE)
 ...
 */
  ...
  if (debug_flags & RECEIVES) {
	printf("Tick %u. Proc %d got %s frame:  ",
						(unsigned int) tick/DELTA,id,badgood[i]);
	fr(&last_frame);
  }
  return(event);
}
 ...
 */
  ...
  got = write(pwfd, s, FRAME_SIZE);
  if (got != FRAME_SIZE) print_statistics();	/* must be done */

  if (debug_flags & SENDS) {
	printf("Tick %u. Proc %d sent frame: ", (unsigned int) tick/DELTA, id);
	fr(s);
  }
}
...
/* Display statistics. */
  ...
  if (id == 0) {
    if (append_file == 1) {
      fpt = fopen("totdata.txt","a");
      fprintf(fpt,"%d ",nevents);
      fprintf(fpt,"%d %d %d ", (int) timeout_interval/10, (int) pkt_loss/10, (int) garbled/10);
      fprintf(fpt,"%d %d %d %d ", data_sent, data_lost, data_not_lost, data_retransmitted);
      fprintf(fpt,"%d %d %d %d ", good_acks_recd, cksum_acks_recd, good_data_recd, cksum_data_recd);
      fprintf(fpt,"%d %d %d %d ", payloads_accepted, acks_sent, acks_lost, acks_not_lost);
      fprintf(fpt,"%d %d\n",timeouts, ack_timeouts);
/*
      fprintf(fpt,"%d %d %d %d %d %d %d %d\n",pkt_loss/10,timeout_interval/10,
              data_sent,data_lost,data_not_lost,data_retransmitted,
              good_data_recd,cksum_data_recd);
*/
    }
  }
  ...
  int n;
  n = write(mwfd, word, 3*sizeof(int));	/* tell main we are done printing */
  sleep(1);
  exit(0);
}
...
int parse_first_six_parameters(int argc, char *argv[], long *event, int *timeout_interval, int *pkt_loss, int *garbled, int *debug_flags, int *the_file)
{
/* Help function for protocol writers to parse first six command-line
 * parameters that the simulator needs.
 */

  if (argc < 7) {
        printf("Need at least six command-line parameters.\n");
        return(0);
  }
  *event = atol(argv[1]);
  if (*event < 0) {
  ...
  *the_file = atoi(argv[6]);
  if ((*the_file !=0) && (*the_file != 1)) {
        printf("\nDatafile flag must be 0 or 1\n");
        printf("0 if this is a single-run and you do not want to add data to totdata.txt\n");
        printf("1 if it is part of a multiple run and you wish to append data to totdata.txt\n\n");
        return(0);
  }
