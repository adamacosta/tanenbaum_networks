...
int main(int argc, char *argv[])
{
  ...
  if (!parse_first_six_parameters(argc, argv, &event, &timeout_interval,
    ...
    printf ("Usage: p3 events timeout loss cksum debug Datafile-flag\n");
    exit(1);
  }

  printf("\n\n Simulating Protocol 3\n");
  start_simulator(sender3, receiver3, event, timeout_interval, pkt_loss, garbled, debug_flags, multiples);
}
