 ...
 *
 *   the_file:  determines whether this is just a single-run of the program or
 *              part of a multi-run.  It affects whether the file totdata.txt is
 *              appended.  (Multi-run causes it to be appended.)  This is all in
 *              relation to the TCL GUI that displays performance graphs.
 *              Values allowed:
 *              0	single-run, do not append to totdata.txt
 *              1	multi-run, DO append to totdata.txt
 */
void start_simulator(void (*proc1)(), void (*proc2)(), long event,
                     int tm_out, int pk_loss, int grb, int d_flags, int the_file);
...
/* Help function for protocol designers to parse first six command-line
 * parameters that the simulator needs. This assumes that the first six
 ...
 */
int parse_first_six_parameters(int argc, char *argv[], long *event,
