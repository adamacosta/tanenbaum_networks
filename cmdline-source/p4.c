 ...
 * To run: protocol4 events timeout  pct_loss  pct_cksum  debug_flags multiple
 *
...
main (int argc, char *argv[])
{
  ...
  if (!parse_first_six_parameters(argc, argv, &event, &timeout_interval,
    ...
    printf ("Usage: p4 events timeout loss cksum debug Datafile-flag\n");
    exit(1);
  }

  printf("\n\n Simulating Protocol 4\n");
  start_simulator(protocol4, protocol4, event, timeout_interval, pkt_loss, garbled, debug_flags, multiples);
}
