...
main (int argc, char *argv[])
{
  ...
  if (!parse_first_six_parameters(argc, argv, &event, &timeout_interval,
    ...
    printf ("Usage: p6 events timeout loss cksum debug Datafile-flag\n");
    exit(1);
  }

  init_max_seqnr(MAX_SEQ + 1);
  printf("\n\n Simulating Protocol 6\n");
  start_simulator(protocol6, protocol6, event, timeout_interval, pkt_loss, garbled, debug_flags, multiples);
}
